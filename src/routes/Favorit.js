...
const { Favorite ,Product } = require("../db.js");
...
router.get("/get/:userId", async (req, res) => {
  ...
  try {
    const userCartItems = await Favorite.findAll({
      where: { userId },
...
router.post("/Post", async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({ error: "no se agrego a favoritos" });
  }
});
...
router.delete("/Delete/:buyId", async (req, res) => {
    ...
    try {
      const buy = await Favorite.findByPk(buyId);
